...
--@스크립트파일이름! 해서 열수있음, 혹은 cmd 창에서 가능!
CREATE OR REPLACE VIEW VIEW_REPORT
AS
SELECT ROWNUM AS RNUM, TITLE, STATUSNAME, REPORTDATE, REPORTERNO, REPORTERNAME, WRITERNO, WRITERNAME, RPCHECK_NO, WHY
FROM
...
-- 신고자, 신고당한자 아이디 나오게 수정본 (수정 완성본 ★★★)
-- + STATUS 0일 시, 미해결이라고 나오게 수정
-- 뷰 순서 : 신고사유, 상태, 신고일시, 신고자(REPORTER), 신고자ID, 원글작성자(WRITER), 원글작성자ID, 신고고유번호, 상세사유
CREATE OR REPLACE VIEW VIEW_REPORT
AS
SELECT ROWNUM AS RNUM, TITLE, STATUSNAME, REPORTDATE, REPORTERNO, REPORTERNAME, WRITERNO, WRITERNAME, RPCHECK_NO, WHY, CHECK_NO
FROM
(
    SELECT W.TITLE AS TITLE, NVL(S.STATUSNAME,'미해결') AS STATUSNAME, R.REPORTDATE AS REPORTDATE, R.CHECK_NO AS CHECK_NO
    , R.AC_NO AS REPORTERNO, P.PE_ID AS REPORTERNAME, C.AC_NO AS WRITERNO, PP.PE_ID AS WRITERNAME, R.RPCHECK_NO AS RPCHECK_NO, R.WHY AS WHY
    FROM REPORT_CHECK R, REPORT_WHY W, REPORT_STATUS S, CHECKLIST C, PERSONAL P, PERSONAL PP
    WHERE R.WHY_NO = W.WHY_NO(+)
      AND R.STATUS_NO = S.STATUS_NO(+)
...
--○ 생성된 뷰 확인 2
SELECT RNUM, TITLE, STATUSNAME, REPORTDATE, REPORTERNO, REPORTERNAME, WRITERNO, WRITERNAME, RPCHECK_NO, WHY
FROM VIEW_REPORT;
------------------------------------
...
INSERT INTO REPORT_CHECK(RPCHECK_NO, REPORTDATE, CHECK_NO, AC_NO, WHY_NO, WHY, CONFIRMDATE) 
VALUES(3, SYSDATE, 1, 16, 2, '도배가 지나쳐요', SYSDATE);
...
-- 신고 처리 게시글 필요 구문
SELECT RNUM, RPCHECK_NO, TITLE, STATUSNAME, REPORTDATE, REPORTERNO, REPORTERNAME, WRITERNO, WRITERNAME, CHECK_NO, WHY
FROM VIEW_REPORT;
